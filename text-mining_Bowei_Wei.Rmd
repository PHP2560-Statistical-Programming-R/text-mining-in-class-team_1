...
hp_wikipedia_sum = readLines('https://en.wikipedia.org/wiki/Harry_Potter_(character)')
#set the summary into a dataframe and name a title called SUM
sum <- data.frame(Title = "SUM", text = hp_wikipedia_sum)
...
custom_stop_words <- c('Wikipedia','Wikimedia','The','Puppet','book','Stories','Harry')

#exclude the custom stop words
hp_wikipedia_sum %>% anti_join(custom_stop_words)

#exclude stop words
hp_wikipedia_sum %>% anti_join(stop_words)

word_count <- hp_wikipedia_sum %>% 
